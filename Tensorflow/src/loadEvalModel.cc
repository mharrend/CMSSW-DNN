...
loadEvalModel::loadEvalModel(std::string modelFileLocation, std::vector<std::string> inputNeurons, std::vector<std::string> outputNeurons)
{
    ...
    dnn::tf::Shape xShape[] = { 1, inputNeurons.size() };
    ...
    m_numberOfOutputNeurons =  outputNeurons.size();

}

void loadEvalModel::evalModel(std::vector<float> eventVariables, std::vector<float> &NNOutput)
{
